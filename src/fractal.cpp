...
fractal::fractal(const int width,const int height):_width(width), _height(height){
	picture = 0;
	engine = 0;
	_active = true;
	_running = false;
...
void fractal::paintEvent(QPaintEvent * event)
 {
     ...
     QPainter painter(this);
		 ...
		 QPixmap current = QPixmap::fromImage(*picture).scaled(width_,height_,Qt::KeepAspectRatio);
...
bool fractal::fromXML(const QDomDocument &frac){
	...
	if (!_funcCount) return false;
	
	for (QDomElement i = func.firstChildElement();!i.isNull();i = i.nextSiblingElement()){
		Function f(i);
		_functions.append(f);
	}
	return true;
...
void fractal::setFuncCount(int count){
	_funcCount = count;
}
void fractal::setIterations(int num){
	_iterations = num;
}
bool fractal::render(){
	stop();
	...
	connect(engine,SIGNAL(nextIteration(int, int, int)),this,SLOT(pointIterated(int, int, int)));
...
bool fractal::getRendered(){
	return rendered;
}
bool fractal::isRunning(){
	return _running;
}
...
void fractal::activated(fractal * active){
	if(_active = active==this){
		...
		if(!engine){
			...
			emit setMax(_iterations); //does not have to match still it is highly probable
	}
}

engineThread::engineThread(const int width, const int height, const int iterations,
...
void engineThread::sendStatus(){
	emit nextIteration(0,_i,_iterations);
}

void engineThread::put(const double x, const double y, const double r, const double g, const double b){
	int pos = (int)((x+1)*(_width/2)) + (int)((y+1)*(_height/2))*_width;
	if(pos < 0|| pos >= (_width * _height) ) return;
	red[pos]	 += r;
	green[pos] += g;
	blue[pos]	 += b;
	hits[pos]  += 1;
...
QColor engineThread::get(const int pos) const{
	if (!red||!green||!blue||!hits||hits[pos]==0)
		return QColor(0,0,0);
	return QColor((red[pos]/hits[pos])*255,(green[pos]/hits[pos])*255,(blue[pos]/hits[pos])*255);
}
int engineThread::getWidth()const{
	return _width;
}
int engineThread::getHeight()const{
	return _height;
}
