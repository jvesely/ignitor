...
void mainWindow::openXML(){
	...
	if (!((fractal*)tabs->currentWidget())->loadXML(fileName)){
		QMessageBox::warning(this,tr("Loading failed!"),tr("Ignitor was unable to load saved fractal!"));
		closeTab();
	}else{
...
void mainWindow::savePict(){
	QString fileName = QFileDialog::getSaveFileName(this, tr("Save Fractal Picture"),
                                                 ".",
                                                 tr("Picture files (*.png *.jpg)"));
  if (fileName.isEmpty())
		return;
  if(!getCurrentFractal()->savePict(fileName))
		QMessageBox::warning(this,tr("Saving failed!"),tr("Ignitor was unable to save fractal picture!"));
}
...
void mainWindow::newFile(){
	...
	connect(this,SIGNAL(activate(fractal *)),nextFr,SLOT(activated(fractal*)));
